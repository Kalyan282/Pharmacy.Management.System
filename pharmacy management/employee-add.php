			...
			<form action="<?=$_SERVER['PHP_SELF']?>" method="post">
				...
				<div class="column">
					...
					<p>
						...
						<input type="number" name="esal" min="1" required>
					</p>
					<p>
    <label for="ephno">Phone Number:</label><br>
    <input type="number" name="ephno" id="ephno" required>
    <span id="ephnoError" style="color:red"></span>
</p>
<p>
    <label for="e_mail">Email Address:</label><br>
    <input type="email" name="e_mail" id="e_mail" required>
    <span id="e_mailError" style="color:red"></span>
</p>

<script>
document.getElementById('ephno').onchange = function() {
    var ephno = this.value;
    if(ephno.length !== 10 || isNaN(ephno)) {
        document.getElementById('ephnoError').textContent = "Phone number must be 10 digits";
    } else {
        document.getElementById('ephnoError').textContent = "";
    }
};

document.getElementById('e_mail').onchange = function() {
    var e_mail = this.value;
    if(e_mail.indexOf('@') === -1) {
        document.getElementById('e_mailError').textContent = "Email must contain an @";
    } else {
        document.getElementById('e_mailError').textContent = "";
    }
};
</script>
